# Portfolio Version 2.2 — AI Engineer Portfolio

A premium, high-performance, and responsive developer portfolio designed to showcase AI Engineering, Machine Learning, and Full-Stack AI capabilities. Built using a pure vanilla stack (HTML5, custom CSS3, and modern ES6 JavaScript) to ensure sub-second page loads, excellent SEO scoring, and zero external dependency bloat. The site is fully responsive for mobile devices, with optimized layouts and navigation for smaller screens.

---

## 🆕 Release Updates (v2.2)

This release introduces a new certifications showcase, navigation additions, and role refinements:

- **New Verified Certifications Page (`certifications.html`)**: Added a dedicated, beautiful certifications showcase displaying 12+ credentials (ALX AI, Oracle OCI AI Foundations, Cisco AI, NASA, NTI Advanced Data Analytics, Eyouth Data Analysis, GDG tracks, and HackerRank Problem Solving) in fully responsive, category-filtered grids. Includes lightbox expansion previews for image-based certs.
- **Title Stabilization (`index.html`)**: Modified the hero section to keep the primary role fixed statically to **AI Engineer** (instead of role cycling typewriter animation) to improve branding clarity.
- **Resume Navigation & Cleanup (`resume.html`)**: Removed the redundant certifications section from the resume page (now unified in the new certifications tab) and added a quick "View Certifications" navigation button next to the resume PDF download button.
- **Cross-Page Nav Updates**: Integrated the new `Certifications` tab across all page navigation headers (`index.html`, `projects.html`, `resume.html`).
